...
Understanding exactly how the cascade works, will help you as a developer to write CSS that is highly scalable.
...
This article is a litle bit long, but after reading it, you’ll have a clearer understanding of how the cascade works, so much that you’ll be able to teach someone else coherently.
...
To decide which coflicting declaration is of higher priority and should be applied, the css cascade goes through the following stages listed below, in ascending order.
...
Here,both conflicting declarations have the same selector weight (`0 1 1`) but the button is be red because the `background-color: red` declaration comes in last in the stylesheet.

This stage also applies for style links in the html `<head>`. If you link more than one stylesheert in your html `<head>`, and there are conflicting declarations from any of the stylesheets, declarations from the last linked stylesheet will be applied. This is why it is advisable to link your own custom styles after linking third party stylesheets e.g bootstrap or Material CSS.

Yippe! you’ve successfully  covered the CSS cascade in detail. Now,  go back to the question asked in the beginning. Can you answer it now?
